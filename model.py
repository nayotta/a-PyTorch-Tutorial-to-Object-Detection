    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        ...
        for i in range(batch_size):
            ...
            for c in range(1, self.n_classes):
                ...
                not_suppress = torch.logical_not(suppress)
                image_boxes.append(class_decoded_locs[not_suppress])
                image_labels.append(torch.LongTensor((not_suppress).sum().item() * [c]).to(device))
                image_scores.append(class_scores[not_suppress])
